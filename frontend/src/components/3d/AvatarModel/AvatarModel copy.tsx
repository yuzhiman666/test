...
const AvatarModelContent = () => {
  ...
  useEffect(() => {
    if (avatarRef.current && scene) {
      ...
      avatarRef.current.position.set(6, -1, 0); 
  ...
  // 对话气泡5秒切换显示/隐藏
  useEffect(() => {
    const timer = setInterval(() => setShowSpeech(prev => !prev), 3000);
    return () => clearInterval(timer);
  }, []);

  // 点击数字人跳转到聊天页面
  // 可以删除这行代码（不再使用）
  // const handleClick = () => navigate('/open-avatar-chat');
  ...
  return (
    ...
    <group 
      ...
      // 原代码：onClick={handleClick}
      onClick={() => window.location.href = 'http://127.0.0.1:8282/ui'} // 修改数字人整体点击事件
          ...
          <Html>
            <div style={{ 
              ...
              <button 
                ...
                onClick={() => window.location.href = 'http://127.0.0.1:8282/ui'}
              >
                立即咨询
              </button>
            </div>
